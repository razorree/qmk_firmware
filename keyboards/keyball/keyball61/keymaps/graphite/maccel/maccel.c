...
bool process_record_maccel(uint16_t keycode, keyrecord_t *record, uint16_t toggle, uint16_t takeoff, uint16_t growth_rate, uint16_t offset, uint16_t limit) {
    if (record->event.pressed) {
        ...
        if (keycode == takeoff) {
            ...
            printf("MACCEL:keycode: TKO: %d gro: %d ofs: %d lmt: %d\n", (int)(g_maccel_config.takeoff*1000), (int)(g_maccel_config.growth_rate*1000), (int)(g_maccel_config.offset*1000), (int)(g_maccel_config.limit*1000));
            return false;
        }
